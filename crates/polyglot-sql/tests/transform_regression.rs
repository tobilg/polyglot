...
// ---------------------------------------------------------------------------
// ANY → IN / always-false rewrite
// ---------------------------------------------------------------------------

fn pg_transpile(sql: &str, target: polyglot_sql::DialectType) -> String {
    polyglot_sql::transpile(sql, polyglot_sql::DialectType::PostgreSQL, target)
...
#[test]
fn any_eq_array_brackets_rewrites_to_in_fabric() {
    let out = pg_transpile(
        "SELECT * FROM t WHERE col = ANY(ARRAY['a', 'b', 'c'])",
        polyglot_sql::DialectType::Fabric,
    );
    assert!(
        out.contains("col IN ('a', 'b', 'c')"),
        "expected IN rewrite, got: {out}"
    );
}

#[test]
fn any_eq_tuple_rewrites_to_in_fabric() {
    let out = pg_transpile(
        "SELECT * FROM t WHERE col = ANY(('a', 'b', 'c'))",
        polyglot_sql::DialectType::Fabric,
    );
    assert!(
        out.contains("col IN ('a', 'b', 'c')"),
...
#[test]
fn any_eq_empty_array_rewrites_to_always_false_fabric() {
    let out = pg_transpile(
        "SELECT * FROM t WHERE col = ANY(ARRAY[])",
        polyglot_sql::DialectType::Fabric,
    );
    assert!(
        out.contains("1 = 0"),
        "expected always-false predicate (1 = 0), got: {out}"
    );
}
...
#[test]
fn any_eq_array_brackets_rewrites_to_in_tsql() {
    let out = pg_transpile(
        "SELECT * FROM t WHERE col = ANY(ARRAY['a', 'b', 'c'])",
        polyglot_sql::DialectType::TSQL,
    );
    assert!(
        out.contains("col IN ('a', 'b', 'c')"),
        "expected IN rewrite, got: {out}"
...
#[test]
fn any_eq_tuple_rewrites_to_in_tsql() {
    let out = pg_transpile(
        "SELECT * FROM t WHERE col = ANY(('a', 'b', 'c'))",
        polyglot_sql::DialectType::TSQL,
    );
...
#[test]
fn any_eq_empty_array_rewrites_to_always_false_tsql() {
    let out = pg_transpile(
        "SELECT * FROM t WHERE col = ANY(ARRAY[])",
        polyglot_sql::DialectType::TSQL,
    );
