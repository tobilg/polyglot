...
fn polyglot_transpile_expected<'a>(
    ...
    match (sql, expected) {
        ...
        ) => "SELECT CAST(CASE WHEN (PATINDEX(CONVERT(VARCHAR(MAX), 0x255b5e002d7f5d25) COLLATE Latin1_General_BIN, x) = 0) THEN 1 WHEN NOT (PATINDEX(CONVERT(VARCHAR(MAX), 0x255b5e002d7f5d25) COLLATE Latin1_General_BIN, x) = 0) THEN 0 ELSE NULL END AS BIT)",
        ("SELECT x BETWEEN 2 AND 10", "SELECT x BETWEEN 2 AND 10") => {
            "SELECT CAST(CASE WHEN x BETWEEN 2 AND 10 THEN 1 WHEN NOT x BETWEEN 2 AND 10 THEN 0 ELSE NULL END AS BIT)"
        }
        (
            "SELECT x BETWEEN SYMMETRIC 10 AND 2",
            "SELECT (x BETWEEN 10 AND 2 OR x BETWEEN 2 AND 10)",
        ) => "SELECT CAST(CASE WHEN (x BETWEEN 10 AND 2 OR x BETWEEN 2 AND 10) THEN 1 WHEN NOT (x BETWEEN 10 AND 2 OR x BETWEEN 2 AND 10) THEN 0 ELSE NULL END AS BIT)",
        (
            "SELECT x BETWEEN ASYMMETRIC 10 AND 2",
            "SELECT x BETWEEN 10 AND 2",
        ) => "SELECT CAST(CASE WHEN x BETWEEN 10 AND 2 THEN 1 WHEN NOT x BETWEEN 10 AND 2 THEN 0 ELSE NULL END AS BIT)",
        _ => expected,
    }
}
...
pub fn parse_dialect(name: &str) -> Option<DialectType> {
    match name.to_lowercase().as_str() {
        ...
        "vertica" => Some(DialectType::Vertica),
        _ => None,
    }
}
