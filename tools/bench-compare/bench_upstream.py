#!/usr/bin/env python3
"""Benchmark mirroring sqlglot's benchmarks/parse.py with the same queries and timing approach.

Supports both parse and transpile modes, comparing polyglot-sql against sqlglot
and optional third-party parsers.

Run via make from the project root:

    make bench-upstream              # parse mode (default)
    make bench-upstream-transpile    # transpile mode

Or directly:

    uv sync --project tools/bench-compare
    uv run --project tools/bench-compare python3 tools/bench-compare/bench_upstream.py
    uv run --project tools/bench-compare python3 tools/bench-compare/bench_upstream.py --mode transpile
    uv run --project tools/bench-compare python3 tools/bench-compare/bench_upstream.py --core-only --quiet
"""
...
# ---------------------------------------------------------------------------
# Third-party parser discovery (subprocess-isolated, handles segfaults)
# ---------------------------------------------------------------------------


def _check_parser(parse_fn, queries):
    """Check which queries a parser can handle, one subprocess per query.
...
def _bench_polyglot(results, mode="parse"):
    """Benchmark polyglot-sql and add to results."""
    fn = polyglot_sql_transpile if mode == "transpile" else polyglot_sql_parse
    for query_name, sql in QUERIES.items():
        key = f"polyglot_sql:{query_name}"
...
def _bench_third_party(results, mode="parse"):
    """Benchmark third-party parsers/transpilers and add to results."""
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Parse/transpile benchmarks (sqlglot-style)")
    parser.add_argument("--json", metavar="FILE", help="Write results as JSON to FILE")
    parser.add_argument("--quiet", action="store_true", help="Suppress progress output")
    parser.add_argument(
        "--core-only",
        action="store_true",
        help="Only benchmark polyglot-sql and sqlglot (skip third-party parsers)",
    )
    parser.add_argument(
        "--mode",
...
if __name__ == "__main__":
    ...
    if not _quiet:
        print(f"=== Benchmarking polyglot-sql ({mode}) ===", flush=True)
    _bench_polyglot(results, mode=mode)

    if not _quiet:
        print(f"\n=== Benchmarking sqlglot ({mode}) ===", flush=True)
    sg_prefix = _bench_sqlglot(results, mode=mode)

    if not args.core_only:
        if not _quiet:
            print(f"\n=== Discovering & benchmarking third-party ({mode}) ===", flush=True)
    ...
    if args.json:
        ...
        _print_table("polyglot_sql", ["polyglot_sql", sg_prefix] + available, results)
