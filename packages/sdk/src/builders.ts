...
function toWasm(value: ExprInput): WasmExprType {
  ...
  if (typeof value === 'number' || typeof value === 'boolean' || value === null) return wasm_lit(value);
  return wasm_null();
}
...
export class Expr {
  ...
  eq(other: ExprInput): Expr { return new Expr(this._w.eq(toWasm(other))); }
  neq(other: ExprInput): Expr { return new Expr(this._w.neq(toWasm(other))); }
  lt(other: ExprInput): Expr { return new Expr(this._w.lt(toWasm(other))); }
  lte(other: ExprInput): Expr { return new Expr(this._w.lte(toWasm(other))); }
  gt(other: ExprInput): Expr { return new Expr(this._w.gt(toWasm(other))); }
  gte(other: ExprInput): Expr { return new Expr(this._w.gte(toWasm(other))); }

  // -- Logical --

  and(other: ExprInput): Expr { return new Expr(this._w.and(toWasm(other))); }
  or(other: ExprInput): Expr { return new Expr(this._w.or(toWasm(other))); }
  not(): Expr { return new Expr(this._w.not()); }
  xor(other: ExprInput): Expr { return new Expr(this._w.xor(toWasm(other))); }

  // -- Arithmetic --

  add(other: ExprInput): Expr { return new Expr(this._w.add(toWasm(other))); }
  sub(other: ExprInput): Expr { return new Expr(this._w.sub(toWasm(other))); }
  mul(other: ExprInput): Expr { return new Expr(this._w.mul(toWasm(other))); }
  div(other: ExprInput): Expr { return new Expr(this._w.div(toWasm(other))); }

  // -- Pattern matching --

  like(pattern: ExprInput): Expr { return new Expr(this._w.like(toWasm(pattern))); }
  ilike(pattern: ExprInput): Expr { return new Expr(this._w.ilike(toWasm(pattern))); }
  rlike(pattern: ExprInput): Expr { return new Expr(this._w.rlike(toWasm(pattern))); }

  // -- Predicates --

  isNull(): Expr { return new Expr(this._w.is_null()); }
  isNotNull(): Expr { return new Expr(this._w.is_not_null()); }
  ...
  alias(name: string): Expr { return new Expr(this._w.alias(name)); }
  as(name: string): Expr { return this.alias(name); }
  cast(to: string): Expr { return new Expr(this._w.cast(to)); }
  asc(): Expr { return new Expr(this._w.asc()); }
  desc(): Expr { return new Expr(this._w.desc()); }

  // -- Output --

  /** Generate SQL string (generic dialect). */
  toSql(): string { return this._w.to_sql(); }
  /** Return the expression AST as a plain JS object. */
  toJSON(): any { return this._w.to_json(); }
  /** Free the underlying WASM handle. */
  free(): void { this._w.free(); }
}
...
export function col(name: string): Expr { return new Expr(wasm_col(name)); }

/** Create a literal value (string, number, boolean, or null). */
export function lit(value: string | number | boolean | null): Expr { return new Expr(wasm_lit(value)); }

/** Create a star (*) expression. */
export function star(): Expr { return new Expr(wasm_star()); }

/** Create a SQL NULL expression. */
export function sqlNull(): Expr { return new Expr(wasm_null()); }

/** Create a SQL boolean literal. */
export function boolean(value: boolean): Expr { return new Expr(wasm_boolean(value)); }

/** Create a table reference. Supports dotted names like `'schema.table'`. */
export function table(name: string): Expr { return new Expr(wasm_table(name)); }

/** Parse a raw SQL fragment into an expression. */
export function sqlExpr(sql: string): Expr { return new Expr(wasm_sql_expr(sql)); }

/** Alias for `sqlExpr()` — parse a SQL condition string. */
export function condition(sql: string): Expr { return sqlExpr(sql); }
...
export function not(expr: ExprInput): Expr { return new Expr(wasm_not(toWasm(expr))); }

/** Create a CAST(expr AS type) expression. */
export function cast(expr: ExprInput, to: string): Expr { return new Expr(wasm_cast(toWasm(expr), to)); }

/** Create an expr AS name alias expression. */
export function alias(expr: ExprInput, name: string): Expr { return new Expr(wasm_alias(toWasm(expr), name)); }
...
export function and(...conditions: ExprInput[]): Expr {
  ...
  if (conditions.length === 1) return conditions[0] instanceof Expr ? conditions[0] : new Expr(toWasm(conditions[0]));
...
export function countDistinct(expr: ExprInput): Expr {
  ...
  const inner = expr instanceof Expr ? expr.toSql() : (typeof expr === 'string' ? expr : String(expr));
  return sqlExpr(`COUNT(DISTINCT ${inner})`);
}
export function sum(expr: ExprInput): Expr { return func('SUM', expr); }
export function avg(expr: ExprInput): Expr { return func('AVG', expr); }
export function min(expr: ExprInput): Expr { return func('MIN', expr); }
export function max(expr: ExprInput): Expr { return func('MAX', expr); }

// -- String functions --

export function upper(expr: ExprInput): Expr { return func('UPPER', expr); }
export function lower(expr: ExprInput): Expr { return func('LOWER', expr); }
export function length(expr: ExprInput): Expr { return func('LENGTH', expr); }
export function trim(expr: ExprInput): Expr { return func('TRIM', expr); }
export function ltrim(expr: ExprInput): Expr { return func('LTRIM', expr); }
export function rtrim(expr: ExprInput): Expr { return func('RTRIM', expr); }
export function reverse(expr: ExprInput): Expr { return func('REVERSE', expr); }
export function initcap(expr: ExprInput): Expr { return func('INITCAP', expr); }
export function substring(expr: ExprInput, start: ExprInput, length?: ExprInput): Expr {
...
export function coalesce(...exprs: ExprInput[]): Expr { return func('COALESCE', ...exprs); }
export function nullIf(expr1: ExprInput, expr2: ExprInput): Expr { return func('NULLIF', expr1, expr2); }
export function ifNull(expr: ExprInput, fallback: ExprInput): Expr { return func('IFNULL', expr, fallback); }

// -- Math functions --

export function abs(expr: ExprInput): Expr { return func('ABS', expr); }
export function round(expr: ExprInput, decimals?: ExprInput): Expr {
  return decimals !== undefined ? func('ROUND', expr, decimals) : func('ROUND', expr);
}
export function floor(expr: ExprInput): Expr { return func('FLOOR', expr); }
export function ceil(expr: ExprInput): Expr { return func('CEIL', expr); }
export function power(base: ExprInput, exp: ExprInput): Expr { return func('POWER', base, exp); }
export function sqrt(expr: ExprInput): Expr { return func('SQRT', expr); }
export function ln(expr: ExprInput): Expr { return func('LN', expr); }
export function exp(expr: ExprInput): Expr { return func('EXP', expr); }
export function sign(expr: ExprInput): Expr { return func('SIGN', expr); }
export function greatest(...exprs: ExprInput[]): Expr { return func('GREATEST', ...exprs); }
export function least(...exprs: ExprInput[]): Expr { return func('LEAST', ...exprs); }

// -- Date/time functions --

export function currentDate(): Expr { return func('CURRENT_DATE'); }
export function currentTime(): Expr { return func('CURRENT_TIME'); }
export function currentTimestamp(): Expr { return func('CURRENT_TIMESTAMP'); }
export function extract(unit: string, from: ExprInput): Expr {
  const fromSql = from instanceof Expr ? from.toSql() : (typeof from === 'string' ? from : String(from));
  return sqlExpr(`EXTRACT(${unit} FROM ${fromSql})`);
}

// -- Window functions --

export function rowNumber(): Expr { return func('ROW_NUMBER'); }
export function rank(): Expr { return func('RANK'); }
export function denseRank(): Expr { return func('DENSE_RANK'); }
...
export function unionAll(left: SelectBuilder, right: SelectBuilder): SetOpBuilder {
  return left.unionAll(right);
}

/** Create an INTERSECT of two SELECT queries. */
export function intersect(left: SelectBuilder, right: SelectBuilder): SetOpBuilder {
  return left.intersect(right);
}

/** Create an EXCEPT of two SELECT queries. */
export function except(left: SelectBuilder, right: SelectBuilder): SetOpBuilder {
